...
export class PaymentService {
  ...
  async getStaffMemberLastPayoutDate(staffMemberId: string) {
    ...
    const lastPayment = await this.databaseService.drizzle.query.payment.findFirst({
      where: (payment, { eq, and }) =>
        and(
          eq(payment.staffMemberId, staffMemberId),
          eq(payment.studioId, this.configService.get('STUDIO_ID')),
          eq(payment.type, PaymentTypeEnum.OUTGOING),
          eq(payment.status, PaymentStatusEnum.COMPLETED),
        ),
